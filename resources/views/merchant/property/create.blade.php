...
<div class="container-fluid card shadow-lg">
        ...
        <form action="{{ route('property.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                ...
                <div class="col">
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <input type="text" class="form-control" name="name" placeholder="Property Name" value="{{ old('name') }}" autofocus>
                        </div>
                    </div>
                    @error('name')
                ...
                <div class="col">
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <input type="number" class="form-control" name="price" placeholder="Property Price" value="{{ old('price') }}" autofocus>
                        </div>
                        @error('price')
                        <small class="text-danger">
                            {{ $message }}
                ...
                <div class="form-group mt-3">
                    ...
                    <textarea name="description" class="form-control" id="" cols="15" rows="5" placeholder="Property Description">{{ old('description') }}</textarea>
                </div>
                @error('description')
                <small class="text-danger">
                    {{ $message }}
            ...
            <div class=" mt-4">
                ...
                <div class="form-group mt-2">
                    ...
                    <textarea id="" cols="30" rows="4" class="form-control" name="landmark" placeholder="What's the nearest landmark in your property?">{{ old('landmark') }}</textarea>
                </div>
                @error('landmark')
                <div class="text-danger">
                    {{ $message }}
            ...
            <div class="mt-4">
                ...
                <div class="row justify-content-center">
                    ...
                    <div class="col-2 me-2 my-2 text-center card pt-3">
                        <i class="{{ $amenity->icons }}" style="font-size: 25px"></i>
    ...
    <div class="mt-4">
        ...
        <div class="form-group row" x-data="addOnsHandler()">
            <template x-for="(addOn, index) in addOns" :key="addOn.id">
                <div class="row pt-3">
                    <div class="col">
                        <label for="">Add-ons</label>
                    </div>
                    <div class="col text-end">
                        <button class="btn btn-danger" type="button" @click="removeAddOns(addOn.id)"><i class="fas fa-trash"></i></button>
                    </div>
                    <div class="row">
                        <div class="col-8">
                            <div class="form-group">
                                <label for="">Name</label>
                                <select name="addons[]" id="" class="select2 form-select">
                                    <option value="" selected disabled>Select Addon</option>
                                    @foreach ($data['addons'] as $addon)
                                    <option value="{{ $addon->id }}" {{ in_array($addon->id, old('addons', [])) ? 'selected' : '' }}>{{ $addon->name }}
                                    </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="col-4">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <input type="number" name="addons_price[]" class="form-control">
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col mt-3 text-end">
                <button @click="addAddOns()" type="button" class="btn btn-primary">New Addon</button>
            </div>
...
<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCubCTZ2gOZAqKfl_pnhwgPrzUW1cnJ5jI&callback=initMap" type="module"></script>
// ALPINE JS
<script defer src="https://cdn.jsdelivr.net/npm/alpinejs@3.x.x/dist/cdn.min.js"></script>
<script>
    function addOnsHandler() {
        let nextId = 1; // Start with an initial ID
        ...
        return {
            ...
            , addAddOns() {
                const newAddOn = {
                    ...
                    , addOnsPrice: 0.0
                , };
                nextId++; // Increment the ID for the next add-on
                this.addOns.push(newAddOn);
            }
            , removeAddOns(id) {
                const indexToRemove = this.addOns.findIndex(addOn => addOn.id === id);
                if (indexToRemove !== -1) {
                    this.addOns.splice(indexToRemove, 1);
                }
            }
        , };
    }
